...
<body>
<div class="title"><h2>修改公文</h2></div>
<form action="" method="post" enctype="multipart/form-data">
<div class="main">
	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><p class="short-input ue-clear">
    	<label>标题：</label>
        <input name="title" type="text" value="<?php echo ($vol["title"]); ?>" placeholder="标题..." />
	</p>
	<input type="hidden" value="<?php echo ($vol["id"]); ?>" name="id">
	<p class="short-input ue-clear">
    	<label>附件：</label>
		<input style="float: left" name="file" type="file"/>
		<span style="float: left;">注意: 如果需要则修改文件 不进行操作则不修改</span>
    </p>
    <p class="short-input ue-clear">
    	<label>作者：</label>
        <input name="author" type="text" value="<?php echo ($vol["author"]); ?>" placeholder="作者..." />
    </p>
    <p class="short-input ue-clear">
		<label>内容：<script id="editor" name="content" type="text/plain" style="width:900px;height:400px;"><?php echo (htmlspecialchars_decode($vol["content"])); ?></script></label>
